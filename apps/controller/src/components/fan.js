...
export default class Fan {
  ...
  constructor(name, GPIOfanPinNumber) {
    this.IOPin = new IOBase(GPIOfanPinNumber, 'output', false);
    this.#name = name;
  ...
  updateState(newState) {
    ...
    if (newState !== currentState) {
      ...
      this.IOPin.writeIO(newState ? true : false);//also updates IOPin state prop
      this.lastStateChangeMs = Date.now();

      // Emit fan/ events on the central bus
      if (newState) {
        eventEmitter.emit('fan/started', { name: this.getName(), newState: newState ? true : false });
      } else {
        eventEmitter.emit('fan/stopped', { name: this.getName(), newState: newState ? true : false });
      }
    }
  }
