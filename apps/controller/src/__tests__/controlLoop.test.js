...
jest.mock('../services/mqttAgent', () => ({
  ...
  default: { // Mock the default export as well
    client: {
      on: jest.fn(),
      publish: jest.fn(),
      subscribe: jest.fn(),
...
import { startControlLoop, controllerStatus, updateAndBroadcastStatusIfValueChanged } from '../controlLoop';
...
describe('controlLoop', () => {
  beforeEach(() => {
    ...
    cfg.get.mockImplementation((key) => {
      switch (key) {
        case 'zone.name': return 'testZone';
        case 'hardware.fan.pin': return 1;
        case 'hardware.heater.pin': return 2;
        case 'hardware.RC.pin': return 3;
        case 'hardware.dhtSensor.pin': return 4;
        case 'hardware.vent.pin': return 5;
        case 'hardware.vent.speedPin': return 6;
        case 'zone.highSetpoint': return 25;
        case 'zone.lowSetpoint': return 20;
        case 'mqtt.brokerUrl': return 'mqtt://localhost';
        case 'mqtt.LWTTopic': return 'test/LWT';
        case 'telemetry.interval': return 60000;
        case 'zone.periodicPublishIntervalMs': return 300000;
  ...
  describe('updateAndBroadcastStatusIfValueChanged', () => {
    test('should update status and broadcast if value changes', () => {
      ...
      expect(logger.warn).toHaveBeenCalledWith(expect.stringContaining('State changed: temperature = 22'));
    });
    ...
    test('should not update status or broadcast if value is the same', () => {
      ...
      expect(logger.warn).not.toHaveBeenCalledWith(expect.stringContaining('State changed'));
    });
  });
  ...
  describe('startControlLoop', () => {
    ...
    afterAll(() => {
      ...
      if (mqttAgent && mqttAgent.client && typeof mqttAgent.client.end === 'function') {
    ...
    test('should initialize components and set initial irrigationPump status', () => {
      ...
      expect(broadcast).toHaveBeenCalledWith(expect.objectContaining({ irrigationPump: false }));
    });
    ...
    test('should set up event listeners and update status accordingly', () => {
      ...
      expect(broadcast).toHaveBeenCalledWith(expect.objectContaining({ temperature: 23 }));

      // Simulate humidityChanged event
      eventEmitter.emit('humidityChanged', { humidity: 60 });
      expect(controllerStatus.humidity).toBe(60);
      expect(broadcast).toHaveBeenCalledWith(expect.objectContaining({ humidity: 60 }));
      ...
      expect(broadcast).toHaveBeenCalledWith(expect.objectContaining({ light: true, setpoint: 25 }));
      ...
      expect(broadcast).toHaveBeenCalledWith(expect.objectContaining({ ventTotal: 0, ventPower: 0, ventSpeed: 0 }));
      ...
      expect(controllerStatus.ventSpeed).toBe(0);
      expect(broadcast).toHaveBeenCalledWith(expect.objectContaining({ ventTotal: 1, ventPower: 1, ventSpeed: 0 }));
      ...
      expect(broadcast).toHaveBeenCalledWith(expect.objectContaining({ ventTotal: 2, ventPower: 1, ventSpeed: 1 }));
    });
