...
export default class TemperatureSensor extends IOBase {
  ...
  read() {
    ...
    sensor.read(config.hardware.dhtSensor.type, config.hardware.dhtSensor.pin, function (err, temperature, humidity) {
      ...
      if (!err) {
        ...
        console.log(`READ from DHT sensor: temperature ${self.temperature}C, humidity ${self.humidity}%`);
        return sensorData;
  ...
  process() {
    ...
    if (this.hasNewState()) {
      //get value from read()
      console.log(`process NEW  temperature state Reading from DHT sensor: temperature ${data.temperature}C, humidity ${data.humidity}%`);
