...
export default class Light extends IOBase {
  ...
  process() {
    //read light sensor and tx at min intervals
    ...
    if (Date.now() >= this.lastStatePublishedMs + this.publishStateIntervalMs) {
        logger.info("READING REGULAR STATE Light: " + this.getState());
    ...
    if (Date.now() >= this.lastSensorReadTimeMs + this.sensorReadIntervalMs) {
        logger.info("READING SENSOR STATE Light: " + this.getState());
  ...
  initiateGetRCChargeLoopCount() {
    ...
    if (Gpio.accessible) {
      ...
      logger.log(
        "error",
        `DEMO-Gpio not accessible returning default #RCLoopCount: ${
          this.#RCLoopCount
        }`
      );
    }
  ...
  readLDRChargeLoopCount(self) {
    ...
    if (self.#currentlySamplingLightSensor == false) {
      ...
      Logger.log(
        logLevel,
        `!! currently SamplingLight Sensor: ${
          self.#currentlySamplingLightSensor
        }`
      );
    }
  }
